...
// void peeAna(TString infile="podio_output_Pentaquark_hepmc_output_20241113_50GeV_10000evts.hepmc.edm4hep.root") { 1k E = 50 GeV
 void peeAna(TString infile="podio_files/podio_Pentaquark_hepmc_output_20241126E100-200GeV_eta1.9-7_100000evts_ip6_hidiv_275x18.root") {
   //void peeAna(TString infile="podio_files/podio_temp.root") {
  ...
  */
  ...
  // pT vs eta
  TH2D* hProton_gen2d =  new TH2D("hProton_gen2d",";#eta;p_{T} (GeV);",60,0,6,40,0,20);
  TH2D* hProton_recoMatchGen2d = (TH2D*)hProton_gen2d->Clone("hProton_recoMatchGen2d");
  ...
  while(tree_reader.Next()) { // Loop over events
    ...
    for (unsigned int j=0; j<recop_px.GetSize(); j++) {
      ...
      // hRecop_pt->Fill( sqrt(recop_px[j]*recop_px[j] + recop_py[j]*recop_py[j]));
      // hRecop_phi->Fill( recop3V.Phi());	  

      int simid = recop_simID[j];

        TVector3 genMatch3V(genp_px[simid], genp_py[simid], genp_pz[simid]);

        hRecop_p->Fill(recop3V.Mag());   // RECO p 
        hRecop_eta->Fill(recop3V.PseudoRapidity());  // RECO eta
        int thePid = kElse; 
        if ((genp_status[simid]==1)&&(genp_pdg[simid]==2212)) 
            thePid = kProton;
        else if ((genp_status[simid]==1)&&(genp_pdg[simid]==11)) 
            thePid = kElectron;
        else if ((genp_status[simid]==1)&&(genp_pdg[simid]==-11)) 
            thePid = kPositron;
        else 
        ...
        // if (  recop_simID[j] > 4 ) { 
        //     cout << "===" << endl;
        //     cout << "reco p = " << recop3V.Mag() << endl;
        //     cout << "recop_simID = " << recop_simID[j] << endl;
        //     cout <<"# of gen particles = " << genp_pdg.GetSize() << endl;
        //     for(unsigned int i=0; i<genp_pdg.GetSize(); i++){ // Loop over thrown particles
        //         float iGenP = sqrt(genp_px[i]*genp_px[i] + genp_py[i]*genp_py[i] +  genp_pz[i]*genp_pz[i]);
        //         cout << "particle " << i <<": " << "status, pdg, p = " 
        //              << genp_status[i] << ", " << genp_pdg[i] << ", " << iGenP << " , " << endl;
        //     }  
        // }
    }
	...
	continue;
      ...
      float iGenPt = sqrt(genp_px[i]*genp_px[i] + genp_py[i]*genp_py[i]);
      float iGenP = sqrt(genp_px[i]*genp_px[i] + genp_py[i]*genp_py[i] +  genp_pz[i]*genp_pz[i]);
	...
	hPc_mass_gen->Fill (pc_4vec_gen.M());
      ...
      if (proton_4vec_reco.X() !=0 && ele_4vec_reco.X() !=0 && pos_4vec_reco.X() !=0 ) {
	...
	hJpsi_mass_gen->Fill (jpsi_4vec_gen.M());
      ...
      if ( ele_4vec_reco.X() !=0 && pos_4vec_reco.X() !=0 ) {
  ...
  c1->Divide(3,3);
    c1->cd(1);
    hProton_pt->Draw("hist");
    hProton_pt_match->Draw("same");
    c1->cd(2);
  hProton_eta->Draw("hist");
  hProton_eta_match->Draw("same");
  c1->cd(3);
  hProton_phi->Draw("hist");
  hProton_phi_match->Draw("same");
  
  c1->cd(4);
  hEle_pt->Draw("hist");
  hEle_pt_match->Draw("same");
  c1->cd(5);
  hEle_eta->Draw("hist");
  hEle_eta_match->Draw("same");
  c1->cd(6);
  hEle_phi->Draw("hist");
  hEle_phi_match->Draw("same");
    
  c1->cd(7);
  hPos_pt->Draw("hist");
  hPos_pt_match->Draw("same");
  c1->cd(8);
  hPos_eta->Draw("hist");
  hPos_eta_match->Draw("same");
  c1->cd(9);
  hPos_phi->Draw("hist");
  hPos_phi_match->Draw("same");
